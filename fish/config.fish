...
set -gx PYTHON2_VERSION (pyenv versions | grep -E "^\*?\s*2\.\d+\.\d+\b" | cut -c 3- |  tail -1 | tr -d '[:space:]')
set -gx PYTHON3_VERSION (pyenv versions | grep -E "^\*?\s*3\.\d+\.\d+\b" | cut -c 3- |  tail -1 | tr -d '[:space:]')
